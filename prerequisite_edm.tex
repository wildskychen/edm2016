...
\begin{document}
	...
	\title{Learning and Evaluating Prerequisite Graphs from Student Performance Data}
...
These prerequisite structures are often hand-engineered by subject matter expert in a costly and time-consuming process.
...
In this paper, we introduce Bayesian Network based Prerequisite Discovery (BNPD), 
a novel data-driven pipeline for inferring the prerequisite structure of skill from student performance on test items. 
...
Some of these approaches discover the relationship of items without using latent variables.
That is, the prerequisites do not use skill mappings, and only find dependencies between items~\cite{desmarais2006learned,vuong2010method,pavlik2008using,piech2015deep}.
...
on test items and then used probabilistic association rules mining to determine the prerequisite relationships between each pair of skills.
...
By modeling prerequisite relations as a Bayesian network, Scheines et al. converted the problem of prerequisite structure discovery to a Bayesian network structural learning problem, and proposed an algorithm to find the Bayesian network structures that fit the data \cite{scheines2014discovering}.
...
In this paper, we propose a novel pipeline to infer the prerequisite structures of skills from students' response data on the test items. 
Our approach also uses Bayesian network to model the prerequisite relationships.
...
Since here we are concerned with which direction the edge goes, we simply compare the products of two probabilities and select the direction that is more probable. 
	...
	\end{figure}
		...
		\begin{figure}[!ht]\small
			...
			\end{subfigure}						
			\caption{Contour plot of $log(ratio)$ for various values of $P(S_2=0|S_1=0)$.\label{fig:contours} }
		\end{figure}
...
can we determine the edge $S_1\rightarrow S_2$? %by comparing $P(S_1=1|S_2=1)\cdot P(S_2=0|S_1=0)$ and $P(S_2=1|S_1=1)\cdot P(S_1=0|S_2=0)$?
Equivalently, we use rule $ratio=\frac{P(S_i=1|S_j=1)\cdot P(S_j=0|S_i=0)}{P(S_j=1|S_i=1)\cdot P(S_i=0|S_j=0)}\ge 1$.
...
The rule holds only if $(1-p)(1-r)-p(1-q)\ge 0$. Thus, we can use the rule to orient edges if $(1-p)(1-r)-p(1-q)\ge 0$.
...
The white region in each contour plot is the region where the rule is not applicable. As it is showed, if $P(S_2=0|S_1=0)=q=1$, 
...
$P(S_2=0|S_1=0)$ can be interpreted as the strength of the prerequisite relationship. When the prerequisite relationship is weak, the rule is less applicable 
and it is harder to discover the prerequisite relationship.
...
However, determining orientations of each reversible edge is not independent and may conflict each other.
...
When one edge is oriented, the constraint is propagated to other reversible edges. In practical implementation, we use a different strategy: 
...
%			\end{figure*}
	...
	As a result, $2\_5\rightarrow 2\_6$ and $2\_5\rightarrow 2\_7$ are discovered for \texttt{HED-chap2},
...
%	We also observed a small contradiction in the case of skill 3-2, which exemplifies the limitation of fully data-driven methods. 
	...
	The \emph{fully connected} models was outperformed by the simpler prerequisite models, suggesting overfitting.% might have happened. 
	...
	In this paper, we proposed BNPD, a novel pipeline for learning the prerequisite dependencies of hidden skill variables from students' response data on test items.
	...
	We demonstrated our approach using both synthetic and real-world data under different conditions such as data contains missing values and noises.
	We show the discovered prerequisite models are useful for predicting student performance.
...
%	By contrast, we try to optimize the full structure of the model.
	...
	In future work, it may be wise to do Structural EM learning with edge orientation in one pass. 
	For example, in Structural EM search, we can use a scoring function which combines the BIC score and a term that penalizes the structures that violate the CPT constraint. In such way, the prerequisite structure learning may be more accurate.
