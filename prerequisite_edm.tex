...
\begin{document}
	...
	\title{Joint Discovery of Skill Prerequisite Graphs \\ and Student Model Parameters}
...
Learning a COMMAND model requires student performance data, a mapping of items to skills ($Q$-matrix), and, \hl{optionally?}, minimal domain knowledge encoded as constraints.
These constraints \hl{may?} operationalize that  it is unlikely for a student to master a skill before mastering its prerequisites. 
COMMAND learns the skill prerequisite relations as a Bayesian network (an 
%stimates the causal structure and 
...
in the second stage, it uses the constraints to select a single  network.
Our experiments on simulations and real student data suggest that COMMAND is \hl{significantly?} better than prior methods in the literature.
...
In this paper, we introduce \emph{Combined student Modelling and prerequisite Discovery} (COMMAND), 
a novel \hl{pipeline} for simultaneously discovering prerequisite structure of skills and  student model parameters from student performance data
...
Prior work has investigated %the discovery of prerequisite structures from data.
%Many of these approaches discover 
the prerequisite relationship of items without considering their mapping into skills~\cite{desmarais2006learned,vuong2010method}. %,piech2015deep}. I don't think this paper is relevant
...
Unfortunately, prior work with Bayesian networks requires a domain expert to design the prerequisite structures (e.g.,\hl{~\cite{kaser2014beyond}}),
because automatic techniques have not been demonstrated to scale to real student data~\cite{scheines2014discovering}.
We now describe the scalable COMMAND algorithm that discovers a Bayesian network that encodes the prerequisite structures.
