...
\begin{document}
	...
	\title{Joint Discovery of Skill Prerequisite Graphs \\ and Student Model From Student Data}
...
%, and, \hl{optionally?}, minimal domain knowledge encoded as constraints.
...
in the second stage, it uses the domain knowledge that it is unlikely for a student to master a skill before mastering its prerequisites to select a single Bayesian network.
...
%	which provides insights into how prerequisites impact the post-requisite probabilistically. 
	...
	For each chapter, We process the data set to find a subset of items and students that do not have missing values.
	That is,  the datasets we use in COMMAND has students responding to \textit{all} of the  items.
