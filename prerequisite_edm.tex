...
\begin{document}
	...
	\title{ Better Prerequisite Discovery From Data: \\
...
\maketitle
\begin{abstract}
Understanding the prerequisites of skills  is useful for designing curriculum, assessing mastery and for student modeling.
For example,  in student modeling, prerequisites can be used for remediation of student knowledge.
%hand-engineered by subject matter experts in a costly and time-consuming process.
Prerequisites are often encoded as graphs designed by subject matter expert in a costly and time-consuming process.
...
In this paper, we introduce \emph{Combined student Modelling and prerequisite Discovery} (COMMAND), 
a novel algorithm for inferring a prerequisite graph and a student model  from data.
%COMMAND models prerequisite relations as a Bayesian network that can be used for student modeling.
Unlike prior work that evaluates prerequisite graphs using synthetic data, 
we develop a methodology to evaluate prerequisite graphs with data collected from students interacting with a digital learning environment.
Our experiments suggest that COMMAND improves on previously published approaches.
...
Students usually acquire skills in a meaningful sequence which starts from relatively simple concepts and gradually approaches more complex ones. 
...
Among these skills, some are preliminary of others such that they must be mastered before the subsequent concepts can be learned.
For instance, students have to know how to do addition before they learn to do multiplication.
\hl{What is a prerequisite structure?}
In this paper, we use prerequisite structure to refer to the relationships among skills that place strict constraints on the order in which these skills can be acquired. Determining the prerequisite relations among skills is crucial for designing curriculum and for assessing mastery.
...
COMMAND models the prerequisite structure of the skills with a statistical model called Bayesian network.
...
Prior work have investigated the discovery of prerequisite structures within domain models from data.
...
\hl{What's wrong with finding dependencies between items?}
...
Bayesian network is a promising tool to model skill topologies as demonstrated in \cite{kaser2014beyond}. \hl{I think Bob Mislevy had one of the first seminal papers of using BNs in education}
...
In this paper, we propose the \emph{Bayesian Network based Prerequisite Discovery} (COMMAND) pipeline to infer the prerequisite structures of skills from data. 
...
COMMAND learns the prerequisite structure of the skills using data with a statistical model called Bayesian network \cite{pearl2000causality,spirtes2001causation}.
...
Then, the input of COMMAND is a matrix $\mathbf{D}$ with $n \times p$ dimensions, an item to skill mapping.%, and (optionally) some constraints on what content can trigger a remediation.
...
COMMAND relies on a popular machine learning algorithm called Structural Expectation Maximization (EM), which has not been used in educational applications. 
...
\begin{algorithm}
	...
	\caption{The COMMAND algorithm \hl{Try to have parallelism between the algorithm sections and subtitles}  \label{alg:COMMAND}}
\end{algorithm}
...
COMMAND represents the prerequisite structure using Bayesian networks that use latent variables to represent the student knowledge of a skill, 
...
Eventually only one Bayesian network structure stands. This procedure is detailed in the \emph{Orient edges} section of Algorithm~\ref{alg:COMMAND}.  
...
%The BIC score for a Bayesian network model is composed of a likelihood term and a term to penalize the model complexity.
	...
	Synthetic data allows us to study how COMMAND compares to ground truth.
	...
	\begin{enumerate}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt]
		...
		\item How well does COMMAND perform when the student performance data has missing values?
	...
	To evaluate the sensitivity of COMMAND to noise, we synthesize Bayesian networks including a  \emph{StudentAbility} node that takes three possible states (low/med/high). 
	...
	Real-world student data often has missing values, i.e., students do not respond to all items. %Structural EM can also be applied with the missing data. 
	...
	To evaluate how COMMAND performs on data with missing values, we generated data sets of size 1000 with varying fraction of randomly missing values ($10\%$, $20\%$, $30\%$, $40\%$, $50\%$) and used COMMAND to recover the structures from these data sets. Again, the models only contains single-skilled items.  
	...
	However, COMMAND is still able to recover the true structures for Structure 1 and 2 even the data contains up to $30\%$ missing values. 
	...
	The prerequisite structure output from COMMAND are depicted in Figure~\ref{fig:ecpe-result}. The estimated conditional probability tables (CPT) are showed correspondingly.
...
%	which provides insights into how prerequisites impact the post-requisite probabilistically. 
	...
	\subsubsection{HED Data Set}
	 \hl{What is HED? ... Just call it Math.}
...
%	We model the test and homework data using assessment models (\S~\ref{sec:assessment_results}) and learning methods~(\S~\ref{sec:learning_results}), respectively.
	...
	In \texttt{HED-chap2}, six skills \hl{no need to talk about book sections} (book sections) are included and each skill is tested on three to eight items, for a total of 30  items.
...
%			\end{figure*}
	...
	\begin{figure}[!ht]
		...
		\end{subfigure}%
		\caption{Prerequisite structures constructed by COMMAND for HED data sets.}
		\label{fig:hed-structures} 
	\end{figure}			
	...
	These relationships are subset of the set of relationships discovered by COMMAND. 
...
%	We also observed a small contradiction in the case of skill 3-2, which exemplifies the limitation of fully data-driven methods. 
	...
	We compare the Bayesian network models generated from COMMAND with four baseline predictors:
	...
	\begin{figure}[!ht]
		...
		\hl{I still think we need to compare to the French group.  Why not learning a BN with the prerequisite they learn? The reviewers complained about this before}
		}
	\end{figure} 	
	...
	On both \texttt{HED-chap2} and \texttt{HED-chap3} data sets, the COMMAND models outperform the other four models.
	The \emph{fully connected} models are the second best performing models.
	On \texttt{HED-chap2}, COMMAND model has an AUC of $0.803\pm0.008$ and the \emph{fully-connected} model has an AUC of $0.791\pm0.007$ (Figure~\ref{fig:auc-chap2}).
	A paired $t$-test reveals that the AUCs of two models are statistically different with a $p$-value of $0.0022$.
	On \texttt{HED-chap3}, COMMAND model has an AUC of $0.775\pm0.007$ and the \emph{fully-connected} model has an AUC of $0.765\pm0.008$ (Figure~\ref{fig:auc-chap3}).
	...
	In this paper, we proposed COMMAND, a novel pipeline for learning the prerequisite dependencies of latent skill variables from students' response data on test items.
