...
Prior work have investigated the discovery of prerequisite structures from data.
...
Bayesian network is a promising tool to model skill topologies as demonstrated in \cite{mislevy2000bayes,kaser2014beyond}. 
%\hl{I think Bob Mislevy had one of the first seminal papers of using BNs in education}
Bayesian network allows the modeling of full structure of skills and can encode conditional independence relationships between the skills.
...
Scheines and his colleagues did not offer any to discriminate between these Bayesian networks.
...
Then, the input of COMMAND is a matrix $\mathbf{D}$ with $n \times p$ dimensions and an item to skill mapping $Q$-matrix.
...
%	which provides insights into how prerequisites impact the post-requisite probabilistically. 
	...
	The data set, named \texttt{Math}, is from anonymized students interacting with an implementation of a seventh grade math curriculum.
...
%	We model the test and homework data using assessment models (\S~\ref{sec:assessment_results}) and learning methods~(\S~\ref{sec:learning_results}), respectively.
	...
	After filtering, we obtained two data sets, \texttt{Math-chap2} and \texttt{Math-chap3} for Chapter 2 and 3 respectively. 
...
%			\end{figure*}
	...
	$2\_5\rightarrow 2\_6$ and $2\_5\rightarrow 2\_7$ are discovered for \texttt{Math-chap2},
...
%	We also observed a small contradiction in the case of skill 3-2, which exemplifies the limitation of fully data-driven methods. 
	...
	On both \texttt{Math-chap2} and \texttt{Math-chap3} data sets, the COMMAND models outperform the other four models.
