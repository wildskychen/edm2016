...
\begin{algorithm}
	...
	\AddNote{lsmtop}{lsmbot}{right}{\footnotesize Refine models}
	\caption{The BNPD algorithm\label{alg:remind}}
\end{algorithm}
...
Bayesian network theory states that some Bayesian networks are statistically equivalent and thus not distinguishable by the scoring function.
...
A $v$-structure in a Bayesian network $G$ is an ordered triple of nodes $(u,v,w)$ such that $G$ contains the directed edges $u\rightarrow v$ and $w\rightarrow v$ and $u$ and $w$ are not adjacent in $G$. \cite{verma1990equivalence}.}, and can be represented with a complete partially DAG (CPDAG) which consist of a directed edge for every irreversible edge and an undirected edge for every reversible edge\footnote{A CPDAG is also called a \emph{pattern}.}. 
We can discriminate between these equivalent Bayesian networks based the following observation.

\textbf{Rule 1}. $S_1$ is a prerequisite of $S_2$, i.e., $S_1\rightarrow S_2$, if and only if $P(S_1=1|S_2=1)= 1$ and $P(S_2=0|S_1=0)= 1$.

\textbf{Rule 1} says if a skill $S_1$ is the prerequisite of a skill $S_2$,
...
This puts additional constraints on the estimated joint distribution.  
However, in real situations, it is possible for a student to know the post-requisite even he does not know the prerequisite. 
Further, there are always noises in the data. Thus, the two conditional probabilities $P(S_1=1|S_2=1)$ and $P(S_2=0|S_1=0)$ are not exactly but close to 1.
Since equivalent Bayesian networks agree on the skeleton but differ in the orientation of some edges, determining a single Bayesian network structure
is equivalent to determining the orientation of the reversible edges.
...
%\end{figure}
	...
	\begin{figure}[!ht]\small
		...
		\end{subfigure}
		\caption{Two equivalent Bayesian networks representing different prerequisite structures can be discriminated by conditional probability tables.\label{fig:equivnets} }
	\end{figure}
